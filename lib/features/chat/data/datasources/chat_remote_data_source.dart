...
class ChatRemoteDataSourceImpl extends ChatRemoteDataSource {
  ...
  Future<List<ChatRoomDto>> getUserChatRooms() async {
    final response = await _network.call(
      "/api/UserDashboard/GetUserChatRooms", // API endpoint for chat rooms
      RequestMethod.get,
    );
  ...
  Future<List<ChatMessageDto>> getChatRoomMessages(
    ...
    final response = await _network.call(
      "/api/UserDashboard/GetUserChatRoomMessages?ChatroomID=$chatroomId", // API endpoint for messages
      RequestMethod.get,
    );
    return (response.data['data'] as List)
